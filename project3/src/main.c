...
void project3(){
	...
	//Log_t * profiler_log2 = (Log_t *) malloc(sizeof(Log_t));
  ...
  if(LOG!=0){
	...
	/*if(UART0_D==0){
		uint8_t payload_string_nrf[24] = " Testing NRF Registers ";
		main_log->logid = INFO;
		main_log->loglength = 24;
		main_log->payload = payload_string_nrf;
		log_item(main_log);
	*/
    uint8_t status_reg = nrf_read_status();
  	uint8_t status[]="\n\r Status Register ";
  	uint8_t * data_TX_s = status;
  	log_string(data_TX_s);
  	LOG_RAW_INT(status_reg,2);


	nrf_write_config(CONFIG_PWR_UP | CONFIG_PRIM_RX);
	uint8_t config_reg = nrf_read_config();
	uint8_t config[]="\n\r Config Register ";
	uint8_t * data_TX_c = config;
	log_string(data_TX_c);
	LOG_RAW_INT(config_reg,2);

	nrf_write_rf_setup(RF_SETUP_VALUE);
	uint8_t setup_reg = nrf_read_rf_setup();
	uint8_t setup[]="\n\r setup Register ";
	...
	uint8_t channel_reg = nrf_read_rf_ch();
	uint8_t ch[]="\n\r Channel Register ";
	...
	uint8_t fifo_reg = nrf_read_fifo_status();
	uint8_t fifo[]="\n\r Fifo Register ";
	uint8_t * data_TX_f = fifo;
	log_string(data_TX_f);
	...
	log_string(data_TX_tx);
	for(int i=0;i<5;i++){
		LOG_RAW_INT(txaddr_read[i],1);
		log_string(" ");
	}

//destroy_log_item(main_log);
...
#ifdef EXTRA
					memmove_DMA_uart_byte(&data,&(UART0_D),(size_t)1);
#else
					UART0_D=data;
#endif
